...
var explorer_inline_default = `(function(){if(typeof document!=='undefined'){var es=document.getElementsByClassName('explorer');for(var i=0;i<es.length;i++){var b=es[i].querySelector('.mobile-explorer');if(b)b.classList.remove('hide-until-loaded');}}})();function setupMobileAllTags(){
  if(window.innerWidth>800) return;
  var explorer=document.querySelector('.explorer-content');
  if(!explorer) return;
  if(explorer.querySelector('.mobile-all-tags')) return;
  var at=document.querySelector('.left.sidebar > .all-tags');
  if(!at) return;
  var atClone=at.cloneNode(true);
  atClone.removeAttribute('id');
  atClone.classList.add('mobile-all-tags');
  atClone.classList.remove('mobile-only');
  var tocWrap=explorer.querySelector('.mobile-toc');
  if(tocWrap&&tocWrap.nextSibling) explorer.insertBefore(atClone,tocWrap.nextSibling);
  else explorer.appendChild(atClone);
  atClone.addEventListener('click',function(e){
...
export { Explorer_default as ExplorerFork };
